...
/*bool SpeedTestScreen::checkConfiguration() {
    ...
    // More reliable default URLs that work better with CURL
    // This 10MB test file is hosted on CDN which should be reliable
    ...
    Logger::debug("SpeedTestScreen: Retrieved URL from config: '" + url + "'");
    
    if (!url.empty()) {
        m_downloadUrl = url;
        Logger::debug("SpeedTestScreen: Using configured download URL: " + m_downloadUrl);
    } else {
        Logger::warning("SpeedTestScreen: No download_url configured, using default: " + defaultUrl);
...
void SpeedTestScreen::update() {
    ...
    if (shouldUpdateAnimation && (m_downloadInProgress || m_uploadInProgress)) {
    ...
    if (shouldUpdateProgress && (m_downloadInProgress || m_uploadInProgress)) {
        ...
        if (m_downloadInProgress && m_uploadEnabled) {
            ...
            if (m_testResult == 0) {
                ...
                if (m_downloadSpeed > 0) {
                    ...
                    m_display->drawText(0, 20, speedStream.str());
                    usleep(Config::DISPLAY_CMD_DELAY);
                }
                ...
                if (m_uploadSpeed > 0) {
                    ...
                    m_display->drawText(0, 55, speedStream.str());
                    usleep(Config::DISPLAY_CMD_DELAY);
                }
                
                // Show press button message
                m_display->drawText(15, 45, "Press to exit");
                usleep(Config::DISPLAY_CMD_DELAY);
            }
            else {
                resultText = "Test Failed!";
                m_display->drawText(15, 20, resultText);
                usleep(Config::DISPLAY_CMD_DELAY);
                m_display->drawText(15, 45, "Press to exit");
                usleep(Config::DISPLAY_CMD_DELAY);
            }
