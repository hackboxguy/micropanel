...
void MicroPanel::setupMenu()
{
    ...
    // Add standard menu items with corresponding actions
    //registerModuleInMenu("hello", "Hello World");
    //registerModuleInMenu("counter", "Counter");
    
    // Special case for Invert Display (not a module)
    //m_mainMenu->addItem(std::make_shared<ActionMenuItem>("Invert Display", [this]() {
    //    m_display->setInverted(!m_display->isInverted());
    //}));
