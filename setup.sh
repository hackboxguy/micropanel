...
if [ $VERBOSE -eq 1 ]; then
    ...
    DEBIAN_FRONTEND=noninteractive apt-get install -y cmake libudev-dev nlohmann-json3-dev
else
    DEBIAN_FRONTEND=noninteractive apt-get update < /dev/null > /dev/null
    DEBIAN_FRONTEND=noninteractive apt-get install -y -qq cmake libudev-dev nlohmann-json3-dev < /dev/null > /dev/null
fi
